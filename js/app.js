...
const array = [1,2,3,4,6,710,34013,0,13];
function minVal(array) {return Math.min(...array)}

// 2.функция multiply не принимает явно никаких параметров
function multiply() {
  const arrNum = [...arguments];
  const vals = arrNum.reduce((mult, current ) => {
    return mult * current
  })
  console.log(vals);
}
...
const obj = {
    ...
    showPrediction() {
      let randomIndex = obj.takeNumber();
